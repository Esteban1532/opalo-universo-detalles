...
{/* FOOTER PREMIUM */}
      <footer className="bg-gradient-to-b from-white to-pink-50 border-t border-pink-100 py-12 px-6 mt-16 pb-24 sm:pb-12">
        <div className="max-w-5xl mx-auto flex flex-col items-center text-center space-y-8">
          ...
          {/* Nombre y Ubicación interactiva */}
          <div className="space-y-1">
            ...
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer"
              className="group text-slate-500 hover:text-purple-600 font-bold text-sm flex items-center justify-center gap-2 transition-colors"
          ...
          <div className="flex gap-6 sm:gap-8">
            {[
              ...
              { name: "TikTok", url: "https://www.tiktok.com/@opalo.ud?_r=1&_t=ZS-98rRYhXQSe1", color: "text-slate-800", icon: "M9 0h1.98c.144.715.54 1.617 1.235 2.512C12.895 3.389 13.797 4 15 4v2c-1.753 0-3.077-.814-4-1.829V11a5 5 0 1 1-5-5v2a3 3 0 1 0 3 3z" }
            ].map((social) => (
              <a 
                key={social.name}
                href={social.url} 
                ...
                title={social.name}
              >
                <div className={`w-12 h-12 rounded-full bg-white shadow-md border border-pink-100 flex items-center justify-center ${social.color} transition-all duration-300 group-hover:scale-110 group-hover:shadow-pink-200 group-hover:-translate-y-1`}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 16 16"><path d={social.icon}/></svg>
                </div>
                <span className="absolute -bottom-6 opacity-0 group-hover:opacity-100 transition-opacity text-[9px] font-black uppercase text-pink-500 tracking-widest bg-white px-2 py-0.5 rounded-full border border-pink-100 shadow-sm pointer-events-none">
                  {social.name}
                </span>
              </a>
            ))}
